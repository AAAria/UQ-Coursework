...
if (round=="evaluation"){
  ...
  train_set <- fread("sales_train_evaluation_household.csv")
  test_set <- fread("sales_test_evaluation_household.csv")
  weights <- fread("weights_evaluation_household.csv")
}else{
...
### add predictions
prediction = read.csv(file = "Submission_1.csv")
prediction = pre[,2:ncol(pred)]
...
for (tsid in 1:nrow(weights)){
  ...
  if (weights$Level_id[tsid]=="Level12"){
    ...
    frc <- as.numeric(prediction[tsid,])  #####Produce forecasts
